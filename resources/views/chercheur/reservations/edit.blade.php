...
@section('content')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('chercheur.reservations.update', $reservation->id) }}" method="POST">
                @csrf
                @method('PUT')
                {{-- Utilisateur & Labo --}}
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" value="{{ auth()->user()->prenom }} {{ auth()->user()->name }}" readonly>
...
<script>
    ...
    function loadData() {
        ...
        // Charger équipements
        fetch(`/chercheur/equipements-disponibles?laboratoire_id=${laboId}&date=${date}&reservation_id={{ $reservation->id }}`)
            .then(r => r.json())
            .then(data => {
        ...
        // Charger horaires
        fetch(`/chercheur/horaires-disponibles?laboratoire_id=${laboId}&date=${date}&reservation_id={{ $reservation->id }}`)
            .then(r => r.json())
            .then(data => {
