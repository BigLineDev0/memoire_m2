...
@section('content')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('chercheur.reservations.store') }}" method="POST">
                @csrf
                {{-- Utilisateur & Labo --}}
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control"
                            value="{{ auth()->user()->prenom }} {{ auth()->user()->name }}" readonly>
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="laboratoire" name="laboratoire_id" class="form-control" required>
                            ...
                            @foreach ($laboratoires as $lab)
                                <option value="{{ $lab->id }}" data-equipements='@json($lab->equipements)'>
                                    {{ $lab->nom }}
                                </option>
                            @endforeach
                        </select>
                ...
                <a href="{{ route('chercheur.reservations.index') }}" class="btn btn-secondary">Annuler</a>
            </form>
        </div>
    </div>

    {{-- JS pour charger dynamiquement les équipements du labo choisi --}}
    <script>
        ...
        function loadData() {
            ...
            // Charger équipements
            fetch(`/chercheur/equipements-disponibles?laboratoire_id=${laboId}&date=${date}`)
                .then(r => r.json())
                .then(data => {
                    ...
                    data.forEach(e => {
                        ...
                        col.innerHTML = `
                            <div class="form-check">
                                ...
                                <label class="form-check-label ${e.disponible ? '' : 'text-danger'}" for="eq${e.id}">
            ...
            // Charger horaires
            fetch(`/chercheur/horaires-disponibles?laboratoire_id=${laboId}&date=${date}`)
                .then(r => r.json())
                .then(data => {
                    ...
                    data.forEach(h => {
                        ...
                        col.innerHTML = `
                            <div class="form-check">
                                ...
                                <label class="form-check-label ${h.disponible ? '' : 'text-danger'}" for="h${h.id}">
